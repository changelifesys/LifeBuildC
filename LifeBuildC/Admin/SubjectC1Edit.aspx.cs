    ...
    public partial class SubjectC1Edit : System.Web.UI.Page
    {
        ...
        private void PageDataLoad()
        {
            ...
            if (dt.Rows.Count >0)
            {
                ...
                //上課時間
                //C1 一、二課
                if (dt.Select("CategoryID='C112'").Count() > 0)
                {
                    txtSDate12.Text = dt.Select("CategoryID='C112'")[0]["SDate"].ToString();
                    string[] arrDate12 = dt.Select("CategoryID='C112'")[0]["SubTime"].ToString().Split(' ');
                    dropSubTime12.SelectedValue = arrDate12[0];
                    txtSubTime12.Text = arrDate12[1];
                }

                //C1 三、四課
                if (dt.Select("CategoryID='C134'").Count() > 0)
                {
                    txtSDate34.Text = dt.Select("CategoryID='C134'")[0]["SDate"].ToString();
                    string[] arrDate34 = dt.Select("CategoryID='C134'")[0]["SubTime"].ToString().Split(' ');
